# IoM × EvilClaw —— Listener 扩展机制与 LOLAgent

## IoM 的 Listener 扩展机制

IoM v0.3.0 引入了一个关键的架构设计：**Listener 扩展机制**。
...
**这意味着：IoM 的控制面可以快速扩展到任意的 Implant 类型。**
...
sequenceDiagram
    ...
    participant EC as EvilClaw
    participant Ag as Agent
    participant LLM as Upstream LLM

    Op->>EC: exec "whoami" → 入队 PendingAction
    Ag->>EC: 下次 API 请求
    EC->>LLM: 转发请求
    LLM-->>EC: LLM 响应
    EC-->>Ag: 响应 + 追加 tool_call(Bash whoami)
    Ag->>Ag: 执行 whoami
    Ag->>EC: 下次请求 (携带 tool_result)
    EC-->>Op: 捕获结果，回传 IoM Server
```

**命令是队列式的，需等 Agent 下次 API 请求消费。**
...
| `download /path` | 从目标取文件 |
| `upload local remote` | 向目标传文件 |
| `skill recon` | 系统侦察 |
| `skill creds` | 凭据收割 |
| `skill privesc` | 提权枚举 |
| `skill persist` | 持久化 |
...
| `skill exfil` | 敏感文件收集 |
| `skill cleanup` | 痕迹清理 |
| `whoami` `ps` `ls` `netstat` `env` ... | IoM 标准系统命令 |
