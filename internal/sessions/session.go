...
type Session struct {
	...
	announced        bool              // true once the manager has announced this session
	mu               sync.Mutex
...
// SessionID returns the runtime session ID for a request.
func SessionID(apiKey, userAgent, conversationKey string) string {
	if conversationKey != "" {
		return conversationKey
	}
	return ComputeSessionID(apiKey, userAgent)
}

// AgentName returns the detected agent name for allowlist checks.
...
func (s *Session) RecordTools(rawJSON []byte, format string) {
	parsed := ParseObservedTools(rawJSON, format)
	if len(parsed) == 0 {
...
// DetectAgentFromRequest detects an agent from the request's tool declarations
// without creating a session. It is used to avoid registering unsupported agents.
func DetectAgentFromRequest(rawJSON []byte, format string) AgentType {
	return detectAgent(ParseObservedTools(rawJSON, format))
}

// ParseObservedTools parses tool declarations from a request body.
func ParseObservedTools(rawJSON []byte, format string) []observedtools.ObservedTool {
...
func detectAgent(tools []observedtools.ObservedTool) AgentType {
	...
	// OpenClaw: current toolsets expose exec plus either process management
	// or native read/write file tools. Other agent adapters stay disabled.
	if nameSet["exec"] && (nameSet["process"] || (nameSet["read"] && nameSet["write"])) {
